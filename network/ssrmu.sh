...
#=================================================
# 本脚本是对doubi大神的的脚本进行了部分修改
# 1. 添加端口，用户，密码随机生成功能，方便无脑部署
#=================================================
...
Generate_random_name() {
  ...
  for (( i=0; i<$length; i++ )); do
    random_name+=${characters:$(($RANDOM %$characters_length)):1}
  done
  echo "$random_name"
}
# 随机生成密码
Generate_random_password() {
  local length=$((14 + RANDOM % 7))
  local random_password
  local characters='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+-='
  local characters_length=${#characters}
  for (( i=0; i<$length; i++ )); do
    random_password+=${characters:$(($RANDOM %$characters_length)):1}
  done
...
Set_config_user(){
	...
	read -e -p "(默认: 随机用户):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user=$(Generate_random_name)
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	read -e -p "(默认: 随机密码14-20位):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password=$(Generate_random_password)
